...
def read_config_file():
    ...
    with open("config.json", "w", encoding='utf-8') as f:
        ...
        config_data["created_at"] = datetime.datetime.now().isoformat()  # ERROR: Raw datetime object
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.txt'):
                ...
                with open(file_path, 'r', encoding='utf-8') as f:  # ERROR: No encoding specified
                    content = f.read()
...
def create_sample_data():
    ...
    for filepath, content in sample_files.items():
        with open(filepath, 'w', encoding='utf-8') as f:  # ERROR: No encoding specified
            f.write(content)
...
def save_results(processed_files):
    ...
    summary = {
        ...
        'processed_at': datetime.datetime.now().isoformat(),  # ERROR: Raw datetime object will cause JSON serialization failure
        'files': processed_files
    }
    
    # Save with potential encoding issues
    with open('output/summary.json', 'w', encoding='utf-8') as f:  # ERROR: No encoding specified
        json.dump(summary, f, indent=2)  # Will fail due to datetime object
    
    # Also save as text report
    with open('output/report.txt', 'w', encoding='utf-8') as f:  # ERROR: No encoding specified
